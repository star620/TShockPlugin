...
| 语法                                 |    权限    |           说明            |
|------------------------------------|:--------:|:-----------------------:|
| /song [歌曲名字/索引号] [harp/bell/theaxe]    |   song   | 为自己播放/停止播放歌曲，支持序号或文件名，默认参数为harp  |
| /songall [歌曲名字/索引号] [harp/bell/theaxe] | songall  | 为所有人播放/停止播放歌曲，支持序号或文件名，默认参数为harp |
| /songlist [页码]                          | songlist |        查看歌曲文件列          |
...
### v1.0.8
- 新增支持索引号播放：`/song 1` 或 `/songall 1` 可通过序号播放
- 新增 `/songlist [页码]` 分页功能，每页显示10首歌曲
- 限制只读取 `.txt` 文件
